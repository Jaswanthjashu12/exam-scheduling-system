...
export function updateCourse(id: string, course: Partial<Course>): Course {
...
export function createRoom(room: Room): Room {
  db.prepare('INSERT INTO rooms (id, name, capacity, building, accessible) VALUES (?, ?, ?, ?, ?)')
    .run(room.id, room.name, room.capacity, room.building, room.accessible ? 1 : 0);
  return room;
}

export function updateRoom(id: string, room: Partial<Room>): Room {
  const current = getRoom(id);
  if (!current) throw new Error(`Room with ID ${id} not found`);
  const updated = { ...current, ...room };
  db.prepare('UPDATE rooms SET name = ?, capacity = ?, building = ?, accessible = ? WHERE id = ?')
    .run(updated.name, updated.capacity, updated.building, updated.accessible ? 1 : 0, id);
  return updated;
}
...
export function updateStudent(id: string, student: Partial<Student>): Student {
...
export function getAllInvigilators(): Invigilator[] {
...
export function getInvigilator(id: string): Invigilator | undefined {
...
export function createInvigilator(inv: Invigilator): Invigilator {
...
export function updateInvigilator(id: string, inv: Partial<Invigilator>): Invigilator {
  const current = getInvigilator(id);
...
export function deleteInvigilator(id: string): void {
  db.transaction(() => {
...
export function createScheduleEntry(entry: ScheduleEntry): ScheduleEntry {
...
export function updateScheduleEntry(id: string, entry: Partial<ScheduleEntry>): ScheduleEntry {
...
export function bulkReplaceScheduleEntries(entries: ScheduleEntry[]): ScheduleEntry[] {
...
export function getCollege(): { name: string; examStartDate: string } {
...
export function updateCollege(name: string, examStartDate: string): void {
